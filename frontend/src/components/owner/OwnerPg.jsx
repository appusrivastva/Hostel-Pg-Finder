...
import photo from '../../images/pg_bg_pic.jpg';
import Footer from '../Footer';

export default function OwnerPg() {
  ...
  const handleSubmit = async (event) => {
    ...
    if (validateForm()) {
      try {
        ...
        form_info.append("pg_type", formData.pg_type);
    ...
    <MDBContainer fluid className="d-flex align-items-center justify-content-center bg-image" style={{ backgroundImage: `url(${photo})` }}>
      ...
      <MDBCard className="m-5" style={{ maxWidth: '600px' }}>
        <MDBCardBody className="px-5">
          <h2 className="text-uppercase text-center mb-5">PG Details</h2>
          <MDBInput label="PG Name:" size="lg" id="name" type="text" onChange={handleInputChange} value={formData.name} name='name' />
          ...
          <div className="mb-4">
            <label className="form-label">PG_Type:</label>
            <div className="form-check form-check-inline">
              <input className="form-check-input" type="radio" name="pg_type" id="girls" value="girls" onChange={handleInputChange} checked={formData.pg_type === 'girls'} />
              <label className="form-check-label" htmlFor="girls">Girls</label>
            </div>
            <div className="form-check form-check-inline">
              <input className="form-check-input" type="radio" name="pg_type" id="boys" value="boys" onChange={handleInputChange} checked={formData.pg_type === 'boys'} />
          ...
          <div className="custom-file mb-4">
            <label className="custom-file-label" htmlFor="customFile">PG Pic:</label>
            <input type="file" className="custom-file-input" id="customFile" name="pic" onChange={handlePic} accept="image/*" />
          </div>
